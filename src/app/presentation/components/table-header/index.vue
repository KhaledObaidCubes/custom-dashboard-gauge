  ...
  <template v-if="!isStrategic">
    ...
    <div class="d-flex flex-wrap">
      <div v-if="planId == PlanIdEnum.strategic" class="d-flex mr-3 align-items-center">
        ...
        <v-badge v-else subtle :accent="counters.mainObjectiveCount ? 'primary' : 'dark'" class="p-2">
          {{ uiState.error ? '-' : counters.mainObjectiveCount }}
        </v-badge>
...
const appContext = IoC.DI().resolve<IAppContext>(`appContext`),
  ...
  router = useRouter(),
  isStrategic = computed(() => router.currentRoute.value.name == 'strategic-plan'),
